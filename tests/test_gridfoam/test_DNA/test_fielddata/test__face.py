...
def test_face_field_init():
    ...
    field = FaceField(
        T=1, C=3, N=8, dtype=torch.float32, device=torch.device("cpu")
    )
    assert field.T == 1
    assert field.C == 3
    assert field.N == 8
...
def test_face_field_zeros_like():
    ...
    field1 = FaceField(
        T=2, C=3, N=8, dtype=torch.float64, device=torch.device("cpu")
    )
...
def test_face_field_add():
    ...
    field1 = FaceField(
        T=1, C=1, N=4, dtype=torch.float32, device=torch.device("cpu")
    )
    field2 = FaceField(
        T=1, C=1, N=4, dtype=torch.float32, device=torch.device("cpu")
    )
