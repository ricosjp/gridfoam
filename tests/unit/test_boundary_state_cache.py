"""Boundary states are reused until field data or dependent flux changes."""
...
def test_boundary_state_cache_invalidates_on_data_change(
    ...
    """
    Tensor replacement and in-place writes both trigger a fresh boundary
    evaluation.
    """
    grid = create_grid(channel_config(tmp_path))
...
def test_boundary_state_cache_tracks_bc_dependencies(
    tmp_path: pathlib.Path,
) -> None:
    """
