"""Optimize uniform inlet velocity through a short unrolled PISO trajectory.

Each evaluation advances a few physical time steps and minimizes
(mean_u_x - target)**2. The autograd graph of that finite sequence is
retained; this is not a checkpoint-recompute trajectory adjoint.

Run: python -m examples.optimize.inlet_velocity_transient.run
"""
...
from gridfoam.optimize.transient import PisoStepMap, TransientAlgorithm
...
from examples.optimize.inlet_velocity_transient.output import (  # noqa: E402
    OptimizationRecord,
    save_results,
)
...
def advance(
    mapping: PisoStepMap, design: TensorState, n_time: int
) -> TensorState:
    """Advance ``n_time`` physical steps from the detached initial snapshot."""
...
def main() -> None:
    ...
    # 1. Prepare the PISO step map and the variable inlet boundary.
    grid = manual_run(Path(__file__).with_name("config.yaml"))
    mapping = PisoStepMap(PISO(grid), apply_inlet_velocity)
    logger.info(
        "transient PISO flow: num_cells=%d n_time=%d delta_t=%g",
        grid.num_cells,
        args.n_time,
        mapping.initial_state.delta_t,
    )
    ...
    state = mapping.initial_state.values
