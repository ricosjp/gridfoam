...
LOG_FILE_NAME = "low_re_cube.log"

logger = logging.getLogger("gridfoam.examples.low_re_cube")
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % write_interval == 0 or step == n_steps:
            ...
            save_export_fields_as_vtu(
                grid,
                str(output_dir / f"low_re_cube_{step:04d}.vtu"),
                ugrid=ugrid,
            )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run low-Re cube cases.")
    parser.add_argument(
        "--re",
        required=True,
        type=float,
